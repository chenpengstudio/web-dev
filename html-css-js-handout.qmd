...
* An HTML document consists a series of **elements** with contents enclosed by `<tag>`'s

* At a high level, 
  * `<html> </html>` contains everything of the page
  * `<head> </head>` contains _information_ such as the title and style sheets. These are usually _invisible_.
  * `<body> </body>` contains the contents (text, links, images) we see and interact
* Always complete an HTML element with an _opening_ tag and a _closing_ tag: `<p> </p>`
...
* [JavaScript basics](https://www.geeksforgeeks.org/javascript-cheat-sheet-a-basic-guide-to-javascript/) on Geeks for Geeks
...
| Tag        |    Meaning/Usage |
|:----------:|:----------:|
| `<title>` | title of the docuemnt| 
| `<link rel="stylesheet" href="styles.css">` | include external style sheets| 
...
| `p {}` | all `<p>` elements | 
| `h1 {}` | all `<h1>` elements | 
| e.g., `.fancy {}` | elements of `class="fancy"`| 
| `* {}` | everything| 
...
1. Register a GitHub account on <https://www.github.com>
2. Sign in GitHub, and create an empty **repository** by clicking the `+` at the top left corner (don't change default settings)
3. On the Quick Setup page, click the link [**uploading an existing file**]{style="color:blue"}
4. Add files of your webpage by uploading them.
5. Click [**Commit Changes**]{style="background-color:#22aa22;color:white"}. Now your new online repo has some files.
6. Click **Setting** under your repository name
...
9. Under the **Branch** subsection, click the drop-down (shown as **None** above), and select **main** and **/ (root)**, respectively. **Save**!
   ![](./images/gh-page-main-branch.png)
